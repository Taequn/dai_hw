...
<<>>=
"1) To begin with, we don't know their baseline, so we can't really assess 
the level of change that they went through."

"2) We'd need to see either a control group to compare the level of effect between
...
"(estimated treatment difference vs sitagliptin -0·77% [95% CI -0·92 to -0·62] 
...
<<>>=
aovFunc <- function(rand.n=FALSE, rand.s=FALSE, equal.m=TRUE, n=5){
  if(rand.n){
    n=round(runif(1, min=0, max=200),0)
  }
  
  if(rand.s){
    s=round(runif(1, min=1, max=10),0)
  }else{
    s=1
  }
  ...
  if(equal.m){
    t1<-rlaplace(n=n, m=0, s=s)
    label1<-"T1"
    
    t2<-rlaplace(n=n, m=0, s=s)
    label2<-"T2"
    
    t3<-rlaplace(n=n, m=0, s=s)
    label3<-"T3"
    
    t4<-rlaplace(n=n, m=0, s=s)
    label4<-"T4"
  }
    ...
    t2<-rlaplace(n=n, m=0, s=s)
    t3<-rlaplace(n=n, m=0, s=s)
    t4<-rlaplace(n=n, m=1, s=s)
  }
  
  dat1<-bind_cols(t1, label1)
  dat2<-bind_cols(t2, label2)
  dat3<-bind_cols(t3, label3)
  dat4<-bind_cols(t4, label4)
  
  dat<-bind_rows(dat1, dat2, dat3, dat4)
  
  aov(formula=dat[1]~dat[2])
}
aovFunc()
@

<<>>=
t1<-rlaplace(n=5, m=0, s=1)
@
